...
Task 05 (frontend scaffold) must be complete. Tasks 03+04 (API endpoints) must also be complete so the wizard can fetch machine classes and networks from the backend.

Verify:
```bash
cd frontend && npm run dev  # starts without errors
curl http://localhost:8080/api/v1/machineclasses  # returns JSON
curl http://localhost:8080/api/v1/namespaces/default/networks  # returns JSON
```

## Your job

Implement the Create Machine wizard at `/machines/new`. It is a two-column layout:
- **Left**: accordion-style form cards (Name → Machine Type → OS Image → Network → Storage)
- **Right**: sticky Summary panel showing the current form values

On submit: call `POST /api/v1/namespaces/:ns/machines` then navigate back to `/machines`.

## Files to modify / create

| Action | File |
|--------|------|
| Replace stub | `frontend/src/views/MachineCreateView.vue` |

(No other files needed — the API client in `frontend/src/api/client.ts` already has `machineClasses.list()`, `networks.list()`, and `machines.create()` from Task 05.)

## Step-by-step

### Step 1 — Replace `frontend/src/views/MachineCreateView.vue`
...
With the backend running against ironcore-in-a-box:
...
2. The Machine Type table should show machine classes from the cluster (or "No machine classes found" if cluster is empty)
3. Fill in a name, select type, pick network if available
4. Click "Launch Machine"
5. Should redirect to `/machines` showing the new machine with Pending state

If the cluster has no machine classes yet, the wizard should show "No machine classes found" gracefully — not an error page.

### Step 3 — Commit

```bash
git add frontend/src/views/MachineCreateView.vue
git commit -m "feat: Create Machine wizard with summary panel"
```

## Done criteria

- `/machines/new` renders the accordion wizard without errors
- Summary panel updates live as form fields change
- Submit calls `POST /api/v1/namespaces/:ns/machines` with the correct payload
- On success navigates back to `/machines`
- On error shows the error message inline
- Form validation prevents submitting without name, machine type, and network