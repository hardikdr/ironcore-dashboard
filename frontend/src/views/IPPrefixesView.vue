<template>
  <v-container fluid class="pa-6">
    ...
    <v-card v-else variant="outlined" rounded="lg">
      ...
      <v-data-table :headers="headers" :items="items" :loading="loading" density="compact" hover :no-data-text="loadError ?? 'No IP prefixes'">
...
async function doDelete() {
  deleting.value = true
  try {
    await api.prefixes.delete(nsStore.active, deleteTarget.value)
